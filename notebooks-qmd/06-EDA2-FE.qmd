...
- In order not to have to run what is at the beginning of this document again, I export the following files:
   - **`df_full_complete.parquet`:** database with all the coordinates where there is a flowering record. This database has corrections to `location` tags and has *fixed* or *static* information for each coordinate.
   - **`data_bloom_year.parquet`:** file with information about the response variable `bloom_doy`.
   - **`data_weather_yearly.parquet`:** file with climate information summarized by coordinate and year. It only has information on the 5 coordinates for prediction.
   - **`data_weather_monthly.parquet`:** file with climate information summarized by coordinate, year and month. It only has information on the 5 coordinates for prediction.
   - **`data_photop_coords_yearly.parquet`:** file with photoperiod information summarized by coordinate and year. It only has information on the 5 coordinates for prediction.
   - **`data_photop_coords_monthly.parquet`:** file with photoperiod information summarized by coordinate, year and month. It only has information on the 5 coordinates for prediction.
   - **`df_weather_coords_predict`:** auxiliary file to avoid problems with crossings by longitude and latitude. While developing the analysis I found it problematic to filter latitudes or longitudes, for this reason I decided to keep them aside to use when necessary.
   - **`df_coords_predict.parquet`:** auxiliary file to avoid problems with longitude and latitude crossings. While developing the analysis I found it problematic to filter latitudes or longitudes, for this reason I decided to keep them aside to use when necessary.

```{r}
path_base <- "../external-data/data-models/"

write_parquet(df_full_complete,
              glue("{path_base}data-global-model/df_full_complete.parquet"))

write_parquet(data_bloom_year,
              glue("{path_base}data_bloom_year.parquet"))

write_parquet(
  data_weather_yearly,
  glue("{path_base}data-local-model/data_weather_yearly.parquet")
)

write_parquet(
  data_weather_monthly,
  glue("{path_base}data-local-model/data_weather_monthly.parquet")
)

write_parquet(
  data_photop_coords_yearly,
  glue(
    "{path_base}data-local-model/data_photop_coords_yearly.parquet"
  )
)
...
write_parquet(
  data_photop_coords_monthly,
  glue(
    "{path_base}data-local-model/data_photop_coords_monthly.parquet"
  )
)

write_parquet(df_coords_predict,
              glue("{path_base}df_coords_predict.parquet"))

write_parquet(df_weather_coords_predict,
              glue("{path_base}df_weather_coords_predict.parquet")) 
